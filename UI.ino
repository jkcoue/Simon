...
bool isGameOverScreenFirstTime = true;
bool isInstructionScreenFirstTime = true;
bool gameWin;
long timePassed;
...
static void drawLaunchPage() {
  isInstructionScreenFirstTime = true;
  isGameOverScreenFirstTime = true;
  gameWin = false;
  OrbitOledMoveTo(7, 2);
...
static void drawInstructionPage() {
  if (isInstructionScreenFirstTime) {
    timePassed = 0;
    isInstructionScreenFirstTime = false;
  }
  OrbitOledMoveTo(0, 0);
...
static void drawGameOverPage() {
  ...
  if (isGameOverScreenFirstTime) {
    timePassed = 0;
    isGameOverScreenFirstTime = false;
  }
  ...
  for (int i = 0; i < Simon.sequenceLength; i++) {
    ...
    switch (Simon.sequence[i])
    {
      ...
      case 5:
        ...
        OrbitOledDrawString("Tilt LEFT");
        RbgOn(7);
        break;
      case 6:
        OrbitOledMoveTo(26, 13);
        OrbitOledDrawString("Tilt RIGHT");
        RbgOn(8);
        break;
    }
    OrbitOledUpdate();
